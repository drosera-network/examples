...
import {IBondFixedExpiryTellerWithDrosera} from "./interfaces/IBondFixedExpiryTellerWithDrosera.sol";
...
contract BondFixedExpiryTellerWithDrosera is BondBaseTeller, IBondFixedExpiryTellerWithDrosera {
    using TransferHelper for ERC20;
    using FullMath for uint256;
    using ClonesWithImmutableArgs for address;

    // Drosera Functionality
    modifier notPaused() {
        require(!isPaused, "BondFixedExpiryTeller: contract is paused");
        _;
    }
    ...
    /* ========== CONSTRUCTOR ========== */
    constructor(address protocol_, IBondAggregator aggregator_, address guardian_, Authority authority_)
    ...
    /// @inheritdoc IBondFixedExpiryTellerWithDrosera
    ...
    /// @inheritdoc IBondFixedExpiryTellerWithDrosera
    function redeem(ERC20BondToken token_, uint256 amount_) external override nonReentrant notPaused {
    ...
    function getBondTokenForMarket(uint256 id_) external view override returns (ERC20BondToken) {
        (,, ERC20 underlying,, uint48 vesting,) = _aggregator.getAuctioneer(id_).getMarketInfoForPurchase(id_);
