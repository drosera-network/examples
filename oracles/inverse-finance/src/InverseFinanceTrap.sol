...
interface IKeep3rV2Oracle {
    function current(address tokenIn, uint256 amountIn, address tokenOut)
        external
        view
        returns (uint256 amountOut, uint256 lastUpdatedAgo);
}
...
contract InverseFinanceTrap {
    ...
    IKeep3rV2Oracle oracle = IKeep3rV2Oracle(0x39b1dF026010b5aEA781f90542EE19E900F2Db15); // Address of the Sushi INV/WETH Keep3rV2Oracle
    ...
    function collect() external view returns (PriceDataPoint memory) {
        (uint256 amountOut, uint256 lastUpdatedAgo) = oracle.current(address(inv), 1 ether, address(weth));

        return PriceDataPoint({price: amountOut, timestamp: lastUpdatedAgo});
    }

    function isValid(PriceDataPoint[] calldata dataPoints) external pure returns (bool) {
        ...
        for (uint256 i = 1; i < len; i++) {
            PriceDataPoint memory prevPrice = dataPoints[i - 1];
            PriceDataPoint memory currentPrice = dataPoints[i];
