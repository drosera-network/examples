 ...
 */
contract ReplicaWithDrosera is Version0, NomadBase {
    // ============ Libraries ============
    ...
    // Drosera Functionality
    modifier notPaused() {
        require(!isPaused, "NomadReplica: contract is paused");
        _;
    }
     ...
     */
    function proveAndProcess(bytes memory _message, bytes32[32] calldata _proof, uint256 _index)
        external
    {
        bytes32 _messageHash = keccak256(_message);
        require(
            acceptableRoot(messages[_messageHash]) || prove(_messageHash, _proof, _index), "!prove"
        );
        process(_message);
    }
     ...
     */
    function process(bytes memory _message) public notPaused returns (bool _success) {
