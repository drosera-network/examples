...
// ============ External Imports ============
import {ECDSA} from "../../../node_modules/@openzeppelin/contracts/cryptography/ECDSA.sol";
...
contract XAppConnectionManager is Ownable {
     ...
     */
    function unenrollReplica(uint32 _domain, bytes32 _updater, bytes memory _signature) external {
        ...
        require(
            Replica(_replica).updater() == TypeCasts.bytes32ToAddress(_updater), "!current updater"
        );
        ...
        address _watcher = _recoverWatcherFromSig(
            _domain, TypeCasts.addressToBytes32(_replica), _updater, _signature
        );
