...
abstract contract BaseBridgeRouter is Version0, Router {
    ...
    function sendToHook(
        ...
        bytes29 _action = BridgeMessage.formatTransferToHook(
            _remoteHook, _amount, _detailsHash, TypeCasts.addressToBytes32(msg.sender), _extraData
        );
        // send message to destination chain bridge router
        _sendTransferMessage(_destination, _tokenId, _action);
        // emit Send event to record token sender
     ...
     */
    function enrollCustom(uint32 _domain, bytes32 _id, address _custom) external onlyOwner {
     ...
     */
    function _sendTransferMessage(uint32 _destination, bytes29 _tokenId, bytes29 _action)
        internal
    {
        ...
        Home(xAppConnectionManager.home()).dispatch(
            _destination, _remote, BridgeMessage.formatMessage(_tokenId, _action)
        );
    }
     ...
     */
    function _handleTransfer(uint32 _origin, uint32 _nonce, bytes29 _tokenId, bytes29 _action)
        internal
    {
     ...
     * @param _action The action
     */
    function _handleTransferToHook(uint32 _origin, uint32 _nonce, bytes29 _tokenId, bytes29 _action)
        internal
    {
     ...
     */
    function _giveLocal(address _token, uint256 _amount, address _recipient) internal virtual {
     ...
     */
    function _giveRepr(address _token, uint256 _amount, address _recipient, bytes32 _detailsHash)
        internal
    {
...
contract EthereumBridgeRouter is BaseBridgeRouter {
     ...
     * @param _recipient The recipient
     */
    function _giveLocal(address _token, uint256 _amount, address _recipient) internal override {
